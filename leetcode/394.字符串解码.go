...
func decodeString(s string) string {
	...
	result := ""
	last := ""
	var dfs func(index int) string
	dfs = func(index int) string {
		if index >= len(stack) {
			return ""
		}
		if stack[index][0] >= 'a' && stack[index][0] <= 'z' {
			if index+1 < len(stack) && stack[index+1][0] >= 'a' && stack[index+1][0] <= 'z' {
				fmt.Println(stack[index])
				return stack[index]
			}
			str := stack[index] + dfs(index+1)
			return str
		}
		temp := ""
		if stack[index][0] >= '0' && stack[index][0] <= '9' {
			for count, _ := strconv.Atoi(stack[index]); count > 0; count-- {
				temp += dfs(index + 1)
			}
		}
		if index+2 < len(stack) && stack[index+2][0] >= 'a' && stack[index+2][0] <= 'z' {
			temp += dfs(index + 2)
		}
		fmt.Println(temp)
		return temp
